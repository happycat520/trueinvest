...
### Now matched to the live dashboard data schema
...
schema this repo's own fetch pipeline (`scripts/fetch/`) produces — the same files
the live dashboards already read.
...
4. **Notable Price Streaks** *(new in v2)* — a short list of `downStreaks`/`upStreaks`
   ...
   already used elsewhere in this project's "high-conviction" banner logic,
   reused here for consistency with the existing product). This uses real, already-computed
...
├── data/
│   ├── dashboard/
│   │   ├── insider.json          ← generated by scripts/fetch/fetch_insider_edgar.py
│   │   ├── key_dates.json        ← generated by scripts/fetch/fetch_key_dates.py
│   │   └── price_movement.json   ← generated by scripts/fetch/fetch_price_movement.py
│   └── digest/
...
**Refreshing `data/dashboard/*.json` going forward:** `.github/workflows/fetch-and-digest.yml`
already handles this — it runs the three fetch scripts on a schedule and commits fresh
JSON before generating the digest, so these files stay current without any manual
copying step.

## 7. Sending the email — reuse what already exists

An earlier iteration of this project already has a working Gmail SMTP send (via
